...
void  mexFunction(int nlhs, mxArray *plhs[],
	...
	int* amg_itmp;
	if(!has_equal_dims(arg_1_par,arg_2_par)) {
		printf("Error: arg_1 and arg_2 should have equal dimensions");
		exit(1);
	}

	out = (void **) mxMalloc( sizeof(void *) * nlhs);
	double* out_1;
	out_1 = create_identical_array_size_fast(arg_2_par,&plhs[0],mxDOUBLE_CLASS);

	double* out_2;
	out_2 = create_identical_array_size_fast(arg_2_par,&plhs[1],mxDOUBLE_CLASS);
